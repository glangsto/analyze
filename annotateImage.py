...
#HISTORY
#16Nov09 GIL add marks for intensity and velocity
...
#                print 'ya,b: %6.1f,%6.1f; slope: %8.4f' % (ya, yb, slope)
                ...
                for iii in range( nData):
                    ...
                    if iii > 50 and iii < (nData-50):
                        if tsky[iii] > tsky[imax]:
                            imax = iii
                obs[nplot] = tsky[imax]
                vmax[nplot] = vel[imax]
...
for iii in range(nplot):
    ax,ay = raDecToPix( ras[iii], decs[iii], nx, ny, transform)
    ixs[iii] = ax
    iys[iii] = ay
...
for iii in range(11):
    radii[iii] = 10
    colorxs[iii] = nx-10
    colorys[iii] = int(((iii-5)*ny/13) + (ny/2))

for iii in range(nplot):
    if obs[iii] > 0:
#        radius[iii] = sqrt(obs[iii]) # obs max in K is radius in pixels
        radius[iii] = obs[iii] # obs max in K is radius in pixels
    else:
        radius[iii] = 1
    if vmax[iii] > 0:
        colorindex = int( vmax[iii]/30.)
        hues[iii] = 6 + colorindex
    else:
        colorindex = int( -vmax[iii]/30.)
        hues[iii] = 5 - colorindex
    ...
    pformat = 'Fit: r,d: %7.2f,%7.2f=>%5.0f,%5.0f t,v: %5.1f,%7.0f r,c: %4.0f,%3.0f'
    if radius[iii] > 10:
        print pformat % (ras[iii], decs[iii], ixs[iii], iys[iii], obs[iii], vmax[iii], radius[iii], hues[iii])

# plot measured peak intensities and velicities as circles
# of various sizes
circles(ixs, iys, radius, c=hues, alpha=0.6, ec='none')

# add velocity indicators and labels
circles(colorxs, colorys, radii, c=colorrange, alpha=0.9, ec='k')
colorvmin = -150
colorvmax = 150

ax1.text( int(nx*.92), colorys[0]-10, ' %7.1f km/s' % (colorvmin), fontsize=11)
ax1.text( int(nx*.92), colorys[5]-10, ' %7.1f km/s' % (0.), fontsize=11)
ax1.text( int(nx*.92), colorys[10]-10, ' %7.1f km/s' % (colorvmax), fontsize=11)
