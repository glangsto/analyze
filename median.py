#Python Script to produce median of a set of observatoins
#HISTORY
#19Jan05 GIL initial version of median spectrum
...
#    print filename
    ...
    datefilestr = tcal.cold.utc.isoformat()
    print datefilestr
    parts = datefilestr.split('T')
    date = parts[0]
    time = parts[1]
        ...
        if ncold > 1:
           ...
           # if time to average (or end of all files)
            if (dt > avetime) or (iii == (nargs-1)) or newObs:
                ...
                cold.write_ascii_file( dirname, outname)
                # convert to MHz
                xv = cold.xdata * 1.E-6
...
# if this was a new obs; restart the sums
        if ncold == 0:
            ...
            ncold = ncold + 1
            sum_time = time.localtime((time.mktime(sum_time)+time.mktime(tcal.cold.utc))
            cold.xdata = tcal.cold.xdata
