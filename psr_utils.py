...
def hist(data, bins, range=None, laby="Number", **kwargs):
    ...
    """
    (ys, lox, dx, out) = np.histogram(data, bins, range)
    xs = np.arange(bins, dtype='d')*dx + lox + 0.5*dx
    maxy = int(1.1*max(ys))
    if maxy < max(ys):
        maxy = max(ys) + 1.0
...
def KS_test(data, cumdist, output=0):
    ...
    D1 = np.maximum.reduce(np.absolute(cumdist(sdata)-
                                         np.arange(nn, dtype='d')/nn))
    D2 = np.maximum.reduce(np.absolute(cumdist(sdata)-
                                         np.arange(1,nn+1, dtype='d')/nn))
    D = max((D1, D2))
...
def rad_to_dms(rad):
    ...
    arc = RADTODEG * np.fmod(np.fabs(rad), PI)
    d = int(arc)
    arc = (arc - d) * 60.0
    m = int(arc)
    s = (arc - m) * 60.0
...
def bins_to_accel(z, T, f=[1.0, 1000.0], device="/XWIN"):
    ...
    if (device):
        Pgplot.plotxy(np.log10(accels), fs, logx=1, logy=1,
                      labx="Frequency (Hz)",
...
def galactic_accel_simple(l, b, D, v_o=240.0, R_o = 8.34):
    ...
    d = D/R_o
    cbcl = np.cos(b*DEGTORAD) * np.cos(l*DEGTORAD)
...
def galactic_accel(l, b, D, v_o=240.0, R_o = 8.34):
    ...
    """
    A_sun = v_o*v_o / (C/1000.0 * R_o*KMPERKPC)
    cb = np.cos(b*DEGTORAD)
    cl = np.cos(l*DEGTORAD)
    sl = np.sin(l*DEGTORAD)
    beta = D/R_o * cb - cl
...
def best_dm_step(maxsmear=0.1, dt=0.00080, dm=0.0, freq=1390.0, numchan=512, chanwidth=0.5):
    ...
    if (tau_tot**2.0 < (tau_chan**2.0+tau_samp**2.0)):
        ...
        return 0.0001205*freq**3.0*2.0/BW*np.sqrt(tau_tot**2.0-tau_chan**2.0-tau_samp**2.0)
...
def pulse_broadening(DM, f_ctr):
    ...
    return 10.0**(-3.59 + 0.129*logDM + 1.02*logDM**2.0 -
                  4.4*np.log10(f_ctr/1000.0))/1000.0
...
def smear_plot(dm=[1.0,1000.0], dmstep=1.0, subdmstep=10.0, freq=1390.0,
    ...
    Pgplot.plotxy(np.log10(dts), ldms, color="green",
                  logx=1, logy=1)
    ppgplot.pgmtxt("b", -6.0, 0.95, 1.0, "Sample Rate")
    Pgplot.plotxy(np.log10(chan_smear), ldms, color="purple",
                  logx=1, logy=1)
    ppgplot.pgmtxt("b", -4.5, 0.95, 1.0, "Channel")
    Pgplot.plotxy(np.log10(BW_smear), ldms, color="red",
                  logx=1, logy=1)
    ppgplot.pgmtxt("b", -3.0, 0.95, 1.0, "Full BW")
    Pgplot.plotxy(np.log10(subband_smear), ldms, color="blue",
                  logx=1, logy=1)
...
def fft_rotate(arr, bins):
    ...
    phasor = np.exp(complex(0.0, TWOPI) * freqs * bins / float(arr.size))
...
def corr(profile, template):
    ...
    """
    return FFT.irfft(FFT.rfft(template) * np.conjugate(FFT.rfft(profile)),
                     profile.size)
...
def spike_profile(N, phase, fwhm):
    ...
    if (mean < 0.5):
        ...
        phsval = np.where(np.less(phsval, mean-0.5),
                               phsval+1.0, phsval)
    return np.where(np.less_equal(phsval, 0.5),
                     np.where(np.less_equal(phsval, peakst),
                               0.0, (phsval - peakst) *
                               normalize * normalize),
                     np.where(np.greater(phsval, peakend),
                               0.0, (1.0 - (phsval - 0.5) *
                                     normalize) * normalize))
...
def expcos_profile(N, phase, fwhm):
    ...
    def fwhm_func(k, fwhm=fwhm):
        if (fwhm < 0.02):
            return np.arccos(1.0-np.log(2.0)/k)/PI-fwhm
        else:
            return np.arccos(np.log(0.5*(np.exp(k)+
                                           np.exp(-k)))/k)/PI-fwhm
    phsval = TWOPI * np.arange(N, dtype='d') / float(N)
    phi = -phase * TWOPI
    if (fwhm >= 0.5):
        return np.cos(phsval + phi) + 1.0
    elif (fwhm < 0.02):
        # The following is from expanding of iO(x) as x->Infinity.
        k = np.log(2.0) / (1.0 - np.cos(PI * fwhm))
        # print "Expansion:  k = %f  FWHM = %f" % (k, fwhm_func(k, 0.0))
        phsval = np.fmod(phsval + phi, TWOPI)
        phsval = np.where(np.greater(phsval, PI),
                           phsval - TWOPI, phsval)
        denom = ((1 + 1/(8*k) + 9/(128*k*k) + 75/(1024*k**3) +
                 3675/(32768*k**4) + 59535/(262144*k**5)) / np.sqrt(TWOPI*k))
        return np.where(np.greater(np.fabs(phsval/TWOPI), 3.0*fwhm), 0.0,
    ...
    if (k < 0.05):
        ...
        return norm * (np.exp(k * np.cos(phsval + phi)) -
                       np.exp(-k))
...
def gaussian_profile(N, phase, fwhm):
    ...
    return np.where(zs<20.0, np.exp(-0.5 * zs**2.0) / \
                     (sigma * np.sqrt(2*np.pi)), 0.0)
...
def coherent_sum(amps):
    ...
    phs0 = phss[0]
    phscorr = phs0 - np.fmod((np.arange(len(amps), dtype='d')+1.0)*phs0, TWOPI)
...
def sigma_power(power):
    ...
    if type(1.0) == type(power):
        if power > 36.0:
            return np.sqrt(2.0 * power - np.log(PI * power))
        else:
            return equivalent_gaussian_sigma(prob_power(power))
    else:
        return np.where(power > 36.0,
                         np.sqrt(2.0 * power - np.log(PI * power)),
...
def power_at_sigma(sigma):
    ...
    """
    return sigma**2 / 2.0 + np.log(np.sqrt(PIBYTWO)
                                      * sigma)
