...
# plot height is the number of channels
h = nData 
# plot width is the number of cold observations
w = ncold
# create an RGB empty image 
data = np.zeros((h, w, 3), dtype=np.uint8)

# now rest the cold count to count average spectra
ncold = 0
...
#                plt.plot(vel, yv, colors[ncolor], linestyle=linestyles[ncolor],label=label)
                colorscale = 30.
                for iii in range( 0, nData):
                    ired = 6.*((nData/2)-iii)/nData
                    if ired < 0:
                        ired = 0
                    elif ired > 1.:
                        ired = 1.
                    iblue = 6.*(iii-(nData/2))/nData
                    if iblue < 0:
                        iblue = 0
                    elif iblue > 1.:
                        iblue = 1.
                    ired = max( min( ired*colorscale*tsky[iii], 255.), 0.)
                    igreen = max( min( colorscale*tsky[iii], 255.), 0.)
                    iblue = max( min( iblue*colorscale*tsky[iii], 255.), 0.)
                    if iii == nplot:
                        print iii, 'RGB: ',ired,igreen,iblue
                    data[iii, nplot, 0] = np.uint8(ired)
                    data[iii, nplot, 1] = np.uint8(igreen)
                    data[iii, nplot, 2] = np.uint8(iblue)
                nplot = nplot + 1
                ncold = 0
