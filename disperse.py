#Python Script to simulate dispersion and dedispersion
#HISTORY
#19APR16 GIL initial version based on fold
...
def dispersion( freq, bw, dm, noise, nChan, nSamples):
    """
    Dispersion produces a simulated time series of samples based on a simulated
    Pulsar pulse.   The Pulse will have unity height before dispersion and 
    Gaussian distributed noise with sigma value _noise_ will be added to the
    samples
    Inputs:
    freq - Frequency of observations (MHz)
    bw   - Bandwidth of observations (MHz)
    dm   - dispersion measure parameter in usual Pulsar units
    noise- 1 sigma values of the noise added to the time series
    nChan- Simulation uses a fourier transform of n Channels to compute to the 
           dispersion effects
    nSamples - number of samples in the simulation.  The Pulse will be in the middle of the
    simulated samples, before dispersion
    """
